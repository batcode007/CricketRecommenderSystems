...
cricket_speciality = []
for cricketer in item:
    cricket_speciality.append([cricketer.Bowler, cricketer.Batsmen, cricketer.WicketKeeper])

cricketer_nationality = []
for cricketer in item:
	cricketer_nationality.append([cricketer.Indian, cricketer.Pakistani, cricketer.SouthAfrican, cricketer.Australian, cricketer.WestIndian, cricketer.SriLankan])
cricket_speciality = np.array(cricket_speciality)
cricketer_nationality=np.array(cricketer_nationality)
cluster = KMeans(n_clusters=5)
cluster.fit_predict(cricket_speciality)
...
for i in range(0, n_users):
    for j in range(0, 5):
        if utility_copy[i][j] == 0:
            ...
            sys.stdout.flush()
            time.sleep(0.00005)
            utility_copy[i][j] = guess(i+1, j+1, 250)
